...
If you're not a KittyCAD employee, you won't be able to access the dev API. You should copy everything from `.env.production` to `.env.development.local` to make your local dev environment point to the production API instead, and add the `VITE_KC_DEV_TOKEN` variable to be set to a real token from https://zoo.dev/account/api-tokens.

Alternatively, when you navigate to `localhost:3000` the easiest way to sign in is to paste `localStorage.setItem('TOKEN_PERSIST_KEY', "<your-token-from-https://zoo.dev/account/api-tokens>")` replacing the with a real token from https://zoo.dev/account/api-tokens of course, then navigate to localhost:3000 again. Note that navigating to localhost:3000/signin removes your token so you will need to set the token again.
