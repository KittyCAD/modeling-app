...
pub trait SketchApi {
    ...
    async fn extend_path(
        &self,
        version: Version,
        sketch: ObjectId,
        path_id: ObjectId,
        segment: PathSegmentCtor,
    ) -> Result<SceneGraphDelta>;
...
pub enum SegmentCtor {
    ...
    // Invariant: Line segment ctor or Arc segments ctor
    Path(Box<SegmentCtor>),
}

#[derive(Debug, Clone, Deserialize, Serialize, ts_rs::TS)]
#[ts(export)]
pub enum PathSegmentCtor {
    Line(PathLineCtor),
    MidPointLine(PathMidPointLineCtor),
    Arc(PathArcCtor),
    ThreePointArc(PathThreePointArcCtor),
    TangentArc(PathTangentArcCtor),
}
...
#[derive(Debug, Clone, Deserialize, Serialize, ts_rs::TS)]
#[ts(export)]
pub struct PathMidPointLineCtor {
    pub midpoint: Point2d<Expr>,
}
...
#[derive(Debug, Clone, Deserialize, Serialize, ts_rs::TS)]
#[ts(export)]
pub struct PathThreePointArcCtor {
    pub end: Point2d<Expr>,
    pub interior: Point2d<Expr>,
}
