...
"#;
    ...
    #[test]
    fn z0006_is_enabled_for_default_lint_consumers() {
        let legacy = crate::Program::parse_no_errs(
            "@settings(kclVersion = 2.0)\nfillet(body, radius = 1mm, tags = [getOppositeEdge(edge1)])",
        )
        .unwrap();
        assert!(
            legacy
        ...
        let migrated = crate::Program::parse_no_errs(
            "@settings(kclVersion = 2.0)\nfillet(body, radius = 1mm, edges = [{ sideFaces = [face1, face2] }])",
        )
        .unwrap();
        assert!(
            migrated
