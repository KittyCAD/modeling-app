...
pub struct FrontendState {
    program: Program,
    scene_graph: SceneGraph,
    /// Lightweight map from engine body IDs to their latest KCL references.
    body_references: HashMap<Uuid, BodyAstReference>,
...
impl FrontendState {
    ...
    pub async fn restore_sketch_checkpoint(
        ...
        self.body_references = body_references_from_execution(
            &self.program.ast,
            &checkpoint.exec_outcome.variables,
            &checkpoint.exec_outcome.artifact_graph,
        );
...
impl SketchApi for FrontendState {
    ...
    async fn new_sketch(
        ...
        let mut plane_ast = sketch_on_ast_expr(&mut new_ast, &self.scene_graph, &self.body_references, &args.on)
            .map_err(KclErrorWithOutputs::no_outputs)?;
...
impl FrontendState {
    ...
    fn update_state_after_exec(&mut self, outcome: ExecOutcome, freedom_analysis_ran: bool) -> ExecOutcome {
        ...
        self.body_references =
            body_references_from_execution(&self.program.ast, &outcome.variables, &outcome.artifact_graph);
...
fn sketch_on_ast_expr(
    ...
    match on {
        ...
        Plane::PrimitiveFace(face) => {
            let body_expr = body_expr_for_engine_id(body_references, face.solid_id).ok_or_else(|| {
                KclError::refactor(format!(
                    "Could not resolve a KCL body for selected primitive face: body_id={}",
                    face.solid_id
                ))
            })?;
            let body_expr = insert_body_of_ast(ast, body_expr, face.body_path.as_deref())?;
            let face_id_expr = create_face_id_ast(body_expr.clone(), face.index);
            Ok(create_face_of_ast(body_expr, face_id_expr))
        }
    }
}

fn body_references_from_execution(
    ast: &ast::Node<ast::Program>,
    variables: &IndexMap<String, KclValueView>,
    artifact_graph: &ArtifactGraph,
) -> HashMap<Uuid, BodyAstReference> {
    let mut references = HashMap::new();

    // In-place operations such as shell reuse their input body's engine ID.
    // Later variables overwrite earlier references so mutations target the result.
    for item in &ast.body {
        let (name, value) = match item {
            ast::BodyItem::VariableDeclaration(declaration) => {
                let name = declaration.declaration.id.name.clone();
                let Some(value) = variables.get(&name) else {
                    continue;
                };
                (name, value)
            }
            ast::BodyItem::ImportStatement(import) => {
                let Some(name) = import.module_name() else {
                    continue;
                };
                // Foreign import aliases are stored as module values, not geometry in
                // `ExecOutcome.variables`. Their artifact supplies the engine ID while
                // the import statement supplies the KCL name.
                let import_range = SourceRange::from(item);
                let Some(imported_id) = artifact_graph.values().find_map(|artifact| match artifact {
                    Artifact::ImportedGeometry(imported) if imported.code_ref.range == import_range => {
                        Some(Uuid::from(imported.id))
                    }
                    _ => None,
                }) else {
                    continue;
        ...
        match value {
            ...
            KclValueView::Tuple { value } | KclValueView::HomArray { value } => {
                for (output_index, entry) in value.iter().enumerate() {
                    let body_id = match entry {
                        KclValueView::Solid { value } => Some(value.id),
                        KclValueView::ImportedGeometry(value) => Some(value.id),
                        _ => None,
                    };
            ...
            KclValueView::ImportedGeometry(value) => {
                references.insert(
                    value.id,
                    BodyAstReference {
                        variable_name: name.clone(),
...
fn body_expr_for_engine_id(body_references: &HashMap<Uuid, BodyAstReference>, body_id: Uuid) -> Option<ast::Expr> {
    let reference = body_references.get(&body_id)?;
    let body_expr = ast_name_expr(reference.variable_name.clone());
    Some(indexed_solid_expr_for_sweep_output(body_expr, reference.output_index))
}
    ...
    match &on_object.kind {
        ObjectKind::Wall(wall) => {
            ...
            let ast::Expr::Name(solid_name_expr) = solid_ref else {
                return Err(KclError::refactor(format!(
                    "Could not resolve solid reference for selected wall: artifact_id={:?}",
            ...
            let ast::Expr::Name(sweep_name_expr) = sweep_ref else {
                return Err(KclError::refactor(format!(
...
fn downstream_composite_code_ref_for_source(artifact_graph: &ArtifactGraph, source_id: ArtifactId) -> Option<&CodeRef> {
    ...
    while visited.insert(current_id) {
        let next_composite_id = downstream_composite_id_for_solid_source(artifact_graph, current_id);

        let Some(composite_id) = next_composite_id else {
            break;
        };
        let Some(Artifact::CompositeSolid(composite)) = artifact_graph.get(&composite_id) else {
            break;
        };
...
fn downstream_composite_id_for_solid_source(
    artifact_graph: &ArtifactGraph,
...
fn count_literal(index: usize) -> ast::Expr {
    ast::Expr::Literal(BoxNode::new(ast::Node::no_src(ast::Literal::from(
        ast::NumericLiteral {
            value: index as f64,
            suffix: NumericSuffix::None,
            raw: index.to_string(),
...
fn insert_body_of_ast(
    ast: &mut ast::Node<ast::Program>,
    body_expr: ast::Expr,
    body_path: Option<&[usize]>,
) -> Result<ast::Expr, KclError> {
    let Some(body_path) = body_path.filter(|path| !path.is_empty()) else {
        return Ok(body_expr);
    };
    let body_name =
        next_free_name_with_padding("body", &find_defined_names(ast)).map_err(|err| KclError::refactor(err.msg))?;
    let body_of_expr = ast::Expr::CallExpressionKw(BoxNode::new(ast::Node::no_src(ast::CallExpressionKw {
        callee: ast_name("bodyOf".to_owned()),
        unlabeled: Some(body_expr),
        arguments: vec![ast::LabeledArg {
            label: Some(ast::Identifier::new("path")),
            arg: ast::Expr::ArrayExpression(BoxNode::new(ast::Node::no_src(ast::ArrayExpression {
                elements: body_path.iter().copied().map(count_literal).collect(),
                digest: None,
                non_code_meta: Default::default(),
            }))),
        }],
    ...
    let body_decl = ast::VariableDeclaration::new(
        ast::VariableDeclarator::new(&body_name, body_of_expr),
        ast::ItemVisibility::Default,
        ast::VariableKind::Const,
    );
    ast.body
...
fn get_or_insert_ast_reference(
    ...
    let ret = match mutate_ast_node_by_source_ref(ast, source_ref, command) {
        Ok((_, ret)) => ret,
        Err(err) => {
            if let Some(var_name) = variable_name_containing_source_ref(ast, source_ref) {
                AstMutateCommandReturn::Name(var_name)
            } else {
                return Err(err);
            }
        }
...
fn variable_name_containing_source_ref(ast: &ast::Node<ast::Program>, source_ref: &SourceRef) -> Option<String> {
    let source_range = match source_ref {
        SourceRef::Simple { range, .. } => *range,
        SourceRef::BackTrace { ranges } => {
            let [range] = ranges.as_slice() else {
                return None;
            };
    ...
    ast.body.iter().find_map(|item| {
        let ast::BodyItem::VariableDeclaration(var_decl) = item else {
            return None;
        };
        let init_range = SourceRange::from(&var_decl.declaration.init);
        let source_is_inside_init = init_range.module_id() == source_range.module_id()
            && init_range.start() <= source_range.start()
            && source_range.end() <= init_range.end();
        if matches!(&var_decl.declaration.init, ast::Expr::SketchBlock(_))
            && init_range != source_range
            && source_is_inside_init
        {
            return None;
        }
        source_is_inside_init.then(|| var_decl.name().to_owned())
    })
}
...
shell001 = shell(extrude001, faces = capEnd001, thickness = 1)";
        ...
        let body_references = body_references_from_execution(&program.ast, &outcome.variables, &outcome.artifact_graph);
    ...
    #[test]
    fn test_new_sketch_on_imported_primitive_index_face() {
        let initial_source = "import \"part.step\" as importedPart\n";
        let program = Program::parse(initial_source).unwrap().0.unwrap();
        let imported_id = Uuid::new_v4();
        let artifact_id = ArtifactId::new(imported_id);
        let import_range = SourceRange::from(&program.ast.body[0]);
        let artifact_graph = ArtifactGraph::from_parts(
            IndexMap::from([(
                artifact_id,
                Artifact::ImportedGeometry(kcl_api::artifact::ImportedGeometryArtifact {
                    id: artifact_id,
                    code_ref: CodeRef::placeholder(import_range),
                    consumed: false,
                }),
            )]),
            1,
        );
        let body_references = body_references_from_execution(&program.ast, &IndexMap::new(), &artifact_graph);
        assert_eq!(body_references[&imported_id].variable_name, "importedPart");
        ...
        let face_expr = sketch_on_ast_expr(
            ...
            &body_references,
            &Plane::PrimitiveFace(crate::frontend::api::PrimitiveFacePlane {
                solid_id: imported_id,
                index: 4,
                body_path: Some(vec![1, 2]),
            }),
        )
        .unwrap();
        let face_decl = ast::VariableDeclaration::new(
        ...
        let source = source_from_ast(&ast);
        assert!(source.contains("body001 = bodyOf(importedPart, path = [1, 2])"));
        assert!(source.contains("face001 = faceOf(body001, face = faceId(body001, index = 4))"));
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn test_new_sketch_on_imported_primitive_face_with_real_engine() {
        let initial_source = r#"@settings(kclVersion = 2.0, experimentalFeatures = allow)
...
"#;
        ...
        let imported_id = frontend
            .body_references
            .iter()
            .find_map(|(id, reference)| (reference.variable_name == "cube").then_some(*id))
            .expect("the import artifact should resolve to its KCL alias");
        let mut ast = frontend.program.ast.clone();
        let face_expr = sketch_on_ast_expr(
            &mut ast,
            &frontend.scene_graph,
            &frontend.body_references,
            &Plane::PrimitiveFace(crate::frontend::api::PrimitiveFacePlane {
                solid_id: imported_id,
                index: 0,
                body_path: None,
        ...
        assert!(source_from_ast(&ast).contains("face001 = faceOf(cube, face = faceId(cube, index = 0))"));

        let sketch_args = SketchCtor {
            on: Plane::PrimitiveFace(crate::frontend::api::PrimitiveFacePlane {
                solid_id: imported_id,
                index: 0,
                body_path: None,
            }),
        };
        let (source_delta, _scene_delta, _sketch_id) = tokio::time::timeout(
            std::time::Duration::from_secs(30),
            frontend.new_sketch(&ctx, ProjectId(0), FileId(0), Version(0), sketch_args),
        )
        .await
        .expect("creating a sketch on an imported BREP face should not hang")
        .unwrap();
        assert!(
            source_delta
                .text
                .contains("face001 = faceOf(cube, face = faceId(cube, index = 0))")
        );
        ctx.close().await;
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn test_delete_imported_primitive_face_with_real_engine() {
        let source = r#"@settings(kclVersion = 2.0, experimentalFeatures = allow)

@(targetRepresentation = brep)
import "cube.step" as cube

selectedFace = faceId(cube, index = 0)
result = deleteFace(cube, faces = selectedFace)
"#;
        let program = Program::parse(source).unwrap().0.unwrap();
        let current_file = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests")
        ...
        let ctx = crate::test_server::new_context(true, Some(current_file), true, program.language_version().unwrap())
            .await
            .unwrap();
        let mut exec_state = crate::ExecState::new(&ctx);

        tokio::time::timeout(std::time::Duration::from_secs(30), ctx.run(&program, &mut exec_state))
            .await
            .expect("deleting an imported BREP face should not hang")
            .unwrap();
        ctx.close().await;
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn test_edge_cuts_on_imported_geometry_with_real_engine() {
        for (operation, size_arg) in [("fillet", "radius"), ("chamfer", "length")] {
            let source = format!(
...
result = {operation}(cube, tags = [selectedEdge], {size_arg} = 1mm)
"#
            );
            let program = Program::parse(&source).unwrap().0.unwrap();
            let current_file = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                ...
                .join("main.kcl");
            let ctx =
                crate::test_server::new_context(true, Some(current_file), true, program.language_version().unwrap())
                    .await
                    .unwrap();
            let mut exec_state = crate::ExecState::new(&ctx);
            ...
            tokio::time::timeout(std::time::Duration::from_secs(30), ctx.run(&program, &mut exec_state))
                .await
                .unwrap_or_else(|_| panic!("{operation} on imported geometry should not hang"))
                .unwrap_or_else(|err| panic!("{operation} on imported geometry should succeed: {err}"));
            ctx.close().await;
        }
    }
...
part = subtract(boxSolid, tools = [cutSolid])
  |> appearance(color = \"#8f96a3\")
";
        let ast = Program::parse(source).unwrap().0.unwrap().ast;
        let line_start = source.find("line").unwrap();
        let line_end = line_start + "line(start = [0, 0], end = [1, 0])".len();
        let line_ref = SourceRef::Simple {
            range: [line_start, line_end, 0].into(),
            node_path: None,
        };
        assert_eq!(variable_name_containing_source_ref(&ast, &line_ref), None);

        let subtract_start = source.find("subtract").unwrap();
        let subtract_end = subtract_start + "subtract(boxSolid, tools = [cutSolid])".len();
        let subtract_ref = SourceRef::Simple {
            range: [subtract_start, subtract_end, 0].into(),
            node_path: None,
        };
        assert_eq!(
            variable_name_containing_source_ref(&ast, &subtract_ref),
            Some("part".to_owned())
        );
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn test_sketch_on_subtracted_sweep_cap_uses_composite_solid() {
        clear_mem_cache().await;
        let source = "\
boxSolid = startSketchOn(XY)
  |> startProfile(at = [0, 0])
  |> line(end = [4, 0], tag = $bottomEdge)
  |> line(end = [0, 4])
  |> line(end = [-4, 0])
  |> close()
  |> extrude(length = 10)
cutSolid = startSketchOn(XY)
  |> startProfile(at = [1, 1])
  |> line(end = [1, 0])
  |> line(end = [0, 1])
  |> line(end = [-1, 0])
...
";
        ...
        let sweep_call_end = sweep_call_start + "extrude(length = 10)".len();
        let part_call_start = source.find("subtract").unwrap();
        let part_call_end = part_call_start + "subtract(boxSolid, tools = [cutSolid])".len();
        let sweep_range = [sweep_call_start, sweep_call_end, 0].into();
        let composite_range = [part_call_start, part_call_end, 0].into();

        let cap_object = frontend
            .scene_graph
            .objects
            .iter()
            .find(|object| {
        ...
        assert!(generated_source.contains("capFace = faceOf(part, face = END)"));
        assert!(!generated_source.contains("faceOf(boxSolid"));
        let ast::Expr::CallExpressionKw(call) = cap_expr else {
