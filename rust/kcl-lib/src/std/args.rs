...
impl Args {
        ...
        T: for<'a> FromKclValue<'a>,
    {
        ...
        let arg = arg.value.coerce(ty, exec_state).ok_or_else(|| {
            let actual_type_name = arg.value.human_friendly_type();
            let msg_base = format!(
                "This function expected the input argument to be of type {ty} but it's actually of type {actual_type_name}"
            );
    ...
    pub(crate) fn get_data_and_sketch<'a, T>(&'a self, exec_state: &mut ExecState) -> Result<(T, Box<Sketch>), KclError>
    where
