...
impl FaceSpecifier {
    ...
    fn body_id(&self) -> Option<Uuid> {
        match self {
            FaceSpecifier::FaceTag(face_tag) => face_tag.body_id(),
            FaceSpecifier::Segment(_) => None,
        }
    }
...
pub(super) async fn make_face(
    ...
    let tagged_body_id = face.body_id();
    let (x_axis, y_axis, units, parent_solid) = match &body {
        ...
        GeometryWithImportedGeometry::ImportedGeometry(imported_geometry) => {
            ...
            let parent_solid = FaceParentSolid::from_imported_geometry(imported_geometry, tagged_body_id);
            (info.x_axis, info.y_axis, exec_state.length_unit(), parent_solid)
        }
