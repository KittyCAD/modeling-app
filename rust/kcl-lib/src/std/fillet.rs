...
fn default_edge_cut_version(kcl_version: KclVersion) -> EdgeCutVersion {
    match kcl_version {
        KclVersion::V1 | KclVersion::V2 => EdgeCutVersion::V1,
        KclVersion::V3 => EdgeCutVersion::V2,
    }
}
...
mod tests {
    ...
    async fn fillet_default_depends_on_kcl_version() {
        ...
        assert_eq!(emitted_fillet_version("3.0", None).await, EdgeCutVersion::V2);
    }
    ...
    async fn explicit_fillet_version_overrides_kcl_default() {
        assert_eq!(emitted_fillet_version("3.0", Some(1)).await, EdgeCutVersion::V1);
    }
