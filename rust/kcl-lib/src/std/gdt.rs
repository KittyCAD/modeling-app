...
async fn inner_profile(
    ...
    for edge in &edges {
        ...
        let meta = vec![Metadata::from(args.source_range)];
        let annotation_id = exec_state.next_uuid();
        let control_frame = datum_control_frame(MbdSymbol::ProfileOfLine, tolerance.to_mm(), &datums);
        let feature_control = AnnotationFeatureControl::builder()
            .entity_id(edge_id)
...
async fn inner_position(
    ...
    for face in &faces {
        ...
        let feature_control = AnnotationFeatureControl::builder()
            ...
            .entity_pos(KPoint2d { x: 0.5, y: 0.5 })
            .leader_type(AnnotationLineEnd::Dot)
            .control_frame(datum_control_frame(MbdSymbol::Position, tolerance.to_mm(), &datums))
            .plane_id(frame_plane.id)
            .offset(if let Some(offset) = &frame_position {
                KPoint2d {
                    x: offset[0].to_mm(),
...
fn datum_control_frame(symbol: MbdSymbol, tolerance: f64, datums: &[char]) -> AnnotationMbdControlFrame {
    match datums {
        [] => AnnotationMbdControlFrame::builder()
            .symbol(symbol)
            .tolerance(tolerance)
