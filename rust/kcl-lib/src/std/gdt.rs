...
/// Maximum allowed length, in characters, of an entity name passed to
/// [`name`].
...
fn name_face_arg_type() -> RuntimeType {
...
pub async fn name(exec_state: &mut ExecState, args: Args) -> Result<KclValue, KclError> {
    let face: Option<NamedFace> = args.get_kw_arg_opt("face", &name_face_arg_type(), exec_state)?;
