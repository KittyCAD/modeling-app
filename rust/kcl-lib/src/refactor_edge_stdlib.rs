//! Refactor fillet/chamfer `tags` that use deprecated edge stdlib calls to `edgeRefs`.
//! Step 3 of refactor-to-edgeRefs: pure AST transform using execution metadata (no re-execution).
...
fn try_replace_fillet_chamfer(call: &mut CallExpressionKw, metadata: &[EdgeRefactorMeta]) {
    ...
    // Replace the tags argument with edgeRefs (same index, change label and value).
    let arg = &mut call.arguments[tags_arg_index];
    arg.label = Some(Identifier::new("edgeRefs"));
...
/// Refactor all fillet/chamfer calls that have deprecated edge stdlib in `tags` to use `edgeRefs`.
