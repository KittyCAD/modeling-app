...
fn export_bindings_stdlib_commands() {
    ...
    // ts-rs owns the structural TypeScript declarations above. The stdlib
    // commands themselves are runtime metadata from walk_stdlib(), so we
    // serialize the value manually and keep `as const` for literal command and
    // argument names.
    let json = serde_json::to_string_pretty(&commands).unwrap();
    let ts_config = ts_rs::Config::default();
    let stdlib_command_arg_shape = StdLibCommandArgShape::decl(&ts_config);
    let stdlib_command_shape = StdLibCommandShape::decl(&ts_config);
    ...
    std::fs::write(
        ...
        format!(
            "// This file was generated by `cargo test -p kcl-lib export_bindings`.\n\
             ...
             export {stdlib_command_arg_shape}\n\n\
             export {stdlib_command_shape}\n\n\
             export const STD_LIB_COMMANDS = {json} as const satisfies Record<string, StdLibCommandShape>\n\n\
