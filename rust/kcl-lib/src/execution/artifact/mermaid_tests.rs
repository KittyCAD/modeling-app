...
#[test]
fn primitive_edge_collision_does_not_replace_existing_artifact() {
    let collision_id = ArtifactId::new(Uuid::new_v4());
    ...
    let artifact = map.get(&collision_id).expect("expected artifact");
    let Artifact::Segment(segment) = artifact else {
        panic!("expected segment to be preserved, got {artifact:?}");
    };
    assert_eq!(segment.path_id, path_id);
}

#[test]
fn primitive_face_collision_does_not_replace_existing_artifact() {
    let collision_id = ArtifactId::new(Uuid::new_v4());
    let sweep_id = ArtifactId::new(Uuid::new_v4());
    let seg_id = ArtifactId::new(Uuid::new_v4());
    let solid_id = ArtifactId::new(Uuid::new_v4());
    let mut map = IndexMap::new();
    ...
    let artifact = map.get(&collision_id).expect("expected artifact");
    let Artifact::Wall(wall) = artifact else {
        panic!("expected wall to be preserved, got {artifact:?}");
    };
    assert_eq!(wall.sweep_id, sweep_id);
    assert_eq!(wall.seg_id, seg_id);
}
