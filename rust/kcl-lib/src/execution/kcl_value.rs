...
impl KclValue {
    ...
    fn coerce_to_union_type(&self, tys: &[PrimitiveType], exec_state: &mut ExecState) -> Option<KclValue> {
        for t in tys {
            if let Some(v) = self.coerce(&RuntimeType::Primitive(t.clone()), exec_state) {
                return Some(v);
            }
        }
...
impl RuntimeType {
    ...
    fn subtype(&self, sup: &RuntimeType) -> bool {
        ...
        match (self, sup) {
            ...
            (Primitive(t1), Union(ts2)) => ts2.contains(t1),
