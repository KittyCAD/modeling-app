...
use crate::{
    ...
    parsing::ast::types::{Node, Program, ProgramLookup},
};
...
fn fill_in_node_paths(artifact: &mut Artifact, programs: &ProgramLookup, cached_body_items: usize) {
    match artifact {
        Artifact::StartSketchOnFace(face) => {
            if face.code_ref.node_path.is_empty() {
                face.code_ref.node_path =
                    NodePath::from_lookup(programs, cached_body_items, face.code_ref.range).unwrap_or_default();
            }
        }
        Artifact::StartSketchOnPlane(plane) => {
            if plane.code_ref.node_path.is_empty() {
                plane.code_ref.node_path =
                    NodePath::from_lookup(programs, cached_body_items, plane.code_ref.range).unwrap_or_default();
            }
        }
        _ => {}
    }
...
fn artifacts_to_update(
    ...
    let node_path = NodePath::from_lookup(programs, cached_body_items, range).unwrap_or_default();
