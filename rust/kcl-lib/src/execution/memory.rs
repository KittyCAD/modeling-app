...
impl ProgramMemory {
    ...
    fn new_env(&self, parent: Option<EnvironmentRef>, is_root_env: bool, owner: usize) -> EnvironmentRef {
        ...
        self.with_envs(|envs| {
            let result = EnvironmentRef(
                envs.len(),
                usize::MAX, /*parent.map(|p| p.1).unwrap_or(usize::MAX)*/
            );
...
impl Stack {
    ...
    /// Update a variable in memory. `key` must exist in memory, if it does this function will panic
...
impl EnvironmentRef {
    ...
    pub fn map_env(&mut self, old: usize, new: usize) {
        if self.0 == old {
            self.0 = new;
        }
    }
