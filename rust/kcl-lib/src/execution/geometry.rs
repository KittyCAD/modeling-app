...
impl FaceParentSolid {
    pub(crate) fn from_imported_geometry(imported_geometry: &ImportedGeometry, body_id: Option<Uuid>) -> Self {
        Self {
            solid_id: body_id.unwrap_or(imported_geometry.id),
...
impl From<&ImportedGeometry> for FaceParentSolid {
    fn from(imported_geometry: &ImportedGeometry) -> Self {
        Self::from_imported_geometry(imported_geometry, None)
    }
}
