...
impl ProgramMemory {
    ...
    pub fn new() -> Arc<Self> {
        Self::new_with_backend(super::MemoryBackendKind::from_env())
    }

    pub(crate) fn new_with_backend(backend: super::MemoryBackendKind) -> Arc<Self> {
        match backend {
            // The frozen selector is intentionally wired to legacy storage until
            // the frozen backend exists. This lets tests and benches exercise
            // the switch path without changing behavior.
            super::MemoryBackendKind::Legacy | super::MemoryBackendKind::Frozen => Self::new_legacy(),
        }
    }
...
impl Stack {
    ...
    pub fn new_for_tests() -> Stack {
        Self::new_for_tests_with_backend(super::MemoryBackendKind::from_env())
    }

    #[cfg(test)]
    pub(crate) fn new_for_tests_with_backend(backend: super::MemoryBackendKind) -> Stack {
