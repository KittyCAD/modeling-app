...
@contextmanager
def _trace_test_backend(
    monkeypatch, *, upgrade=False, header="X-Api-Call-Id", hold=False
):
...
async def test_rejected_handshakes_for_all_engine_entrypoints(
    ...
    path.write_text("x = 1")
    with _trace_test_backend(monkeypatch, header=header):
        for fn, args in _trace_test_calls(path):
...
async def test_metadata_survives_engine_failure_and_deduplicates_handshake(
    ...
    with _trace_test_backend(monkeypatch, upgrade=True, header=header):
        trace = kcl.ApiCallTrace()
        with pytest.raises(kcl.KclError):
...
async def test_trace_retains_id_after_cancellation(monkeypatch):
    with _trace_test_backend(monkeypatch, upgrade=True, hold=True, header=None):
        trace = kcl.ApiCallTrace()
