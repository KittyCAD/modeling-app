...
- Length units:
  ...
  - imperial: `in`, `ft`, `yd`
- Angle units: `deg`, `rad`
- `_` to indicate a zero-dimensional number such as a count or ratio.
...
You can also use `number(Length)`, `number(Angle)`, or `number(Count)`. These types mean a number with any length, angle, or zero-dimensional (count) units, respectively (note that `number(_)` and `number(Count)` are equivalent since there is only one zero-dimensional unit).
