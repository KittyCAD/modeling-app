...
| `edge` | `Edge | any` | The edge to bound. This can be a tagged edge, an edge ID from `edgeId(...)`, or an edge reference object. Edge reference objects use adjacent faces, e.g. `{ sideFaces = [faceTag1, faceTag2], endFaces = [...], index = 0 }`. | Yes |
