...
|----------|------|-------------|----------|
| `edges` | `[BoundedEdge | TaggedEdge | any; 2]` | The two edges that will be blended. Tagged edges blend the full edge length. Edge specifier objects can also be used, e.g. `{ sideFaces = [...], endFaces? = [...], index? = 0 }`. | Yes |
