...
excerpt: "Given a KCL value, returns `true` if the value is a surface and `false` otherwise."
layout: manual
---

Given a KCL value, returns `true` if the value is a surface and `false` otherwise.

```kcl
is_surface(@val: any): bool
```
...
|----------|------|-------------|----------|
| `val` | [`any`](/docs/kcl-std/types/std-types-any) | Value to check if it is a surface or not | Yes |
...
<model-viewer
  class="kcl-example"
  alt="Example showing a rendered KCL program that uses the is_surface function"
  src="/kcl-test-outputs/models/serial_test_example_fn_std-solid-is_surface0_output.gltf"
  ar
  environment-image="/moon_1k.hdr"
  poster="/kcl-test-outputs/serial_test_example_fn_std-solid-is_surface0.png"
  shadow-intensity="1"
  camera-controls
  touch-action="pan-y"
>
