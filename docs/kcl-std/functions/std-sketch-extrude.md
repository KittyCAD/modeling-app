...
| `draftAngle` | [`number(Angle)`](/docs/kcl-std/types/std-types-number) | **Experimental.** Positive draft angle means the sketch gets smaller while extruding, i.e. inwards draft. Negative draft angle means the sketch gets bigger while extruding, i.e. outwards draft. Defaults to zero, i.e. | No |
