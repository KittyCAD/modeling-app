...
|----------|------|-------------|----------|
| `solids` | [[`Solid`](/docs/kcl-std/types/std-types-Solid); 1+] | The distinct solids to use as the base to subtract from. A body cannot also appear in `tools`. | Yes |
| `tools` | [[`Solid`](/docs/kcl-std/types/std-types-Solid)] | The distinct solids to subtract. A body cannot also appear in `solids`. | Yes |
