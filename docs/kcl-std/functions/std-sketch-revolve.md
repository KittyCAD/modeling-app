...
| `axis` | [`Axis2d`](/docs/kcl-std/types/std-types-Axis2d) or [`Edge`](/docs/kcl-std/types/std-types-Edge) | Axis of revolution. Either `axis` or `edgeRef` must be provided, but not both. | No |
| `edgeRef` | [`any`](/docs/kcl-std/types/std-types-any) | Edge reference using face ids (new API). Must be an object with: - `faces`: [Face | Tag; 1+] - Array of faces (or tags referring to faces) that share the edge to use as the axis - `disambiguators?`: [Face | Tag] - Optional array of faces to disambiguate when multiple edges share the same two faces - `index?`: number(Count) - Optional index to select a specific edge when multiple edges share the same faces (0-based) Either `axis` or `edgeRef` must be provided, but not both. | No |
