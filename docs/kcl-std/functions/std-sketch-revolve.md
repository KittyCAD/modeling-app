...
| `symmetric` | [`bool`](/docs/kcl-std/types/std-types-bool) | If true, the revolve will happen symmetrically around the sketch. For revolve, `angle` is the full included angle split across both directions, unlike symmetric extrude where `length` is applied in each direction. Otherwise, the revolve will happen on only one side of the sketch. | No |
