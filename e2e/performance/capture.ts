...
import {
  startAnimationWitness,
  finishAnimationWitness,
} from '@e2e/performance/animation-witness'

const POLL_INTERVAL_MS = 10
const EVENT_TIMING_REPORTING_WINDOW_MS = 1000

export async function startCapture(page: Page) {
  await startAnimationWitness(page)
  try {
    await page.evaluate(() => {
    ...
    await finishAnimationWitness(page)
    throw error
  }
}
  ...
  try {
    ...
    const metadata = {
      ...
      diagnosticInstrumentation:
        'Scoped transition events and animation snapshots',
    ...
    await finishAnimationWitness(page, testInfo)
  }
}
