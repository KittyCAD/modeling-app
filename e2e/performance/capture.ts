...
import {
  finishLayerOwnershipCapture,
  startLayerOwnershipCapture,
} from '@e2e/performance/layer-ownership'
...
export async function startCapture(page: Page) {
  await startLayerOwnershipCapture(page)
  try {
    await page.evaluate(() => {
    ...
    await finishLayerOwnershipCapture(page)
    throw error
  }
}
...
export async function finishCapture(
  ...
  try {
    ...
    try {
      ...
      if (!tronApp)
        throw new Error('Interaction measurements require Electron.')
      ...
      const metadata = {
        ...
        diagnosticInstrumentation: 'CDP layer, owner and compositing queries',
    ...
    await finishLayerOwnershipCapture(page, testInfo)
  }
}
