...
sketch001 = sketch(on = XY) {
  ...
  test('can extrude a selected sketch region then sketch on face', async ({
    page,
    context,
    homePage,
    scene,
...
sketch001 = sketch(on = XZ) {
  ...
  horizontal(line3)
    ...
    await test.step('Expect extruded code', async () => {
      await scene.settled(cmdBar)
      await editor.expectEditor.toContain(
        'region001 = region(point = [0.34mm, 1.3mm], sketch = sketch001)'
      )
      await editor.expectEditor.toContain(
    ...
    await test.step('Expect error on extrude', async () => {
      await expect(
        page.locator('.cm-lint-marker-error').first()
      ).toBeInViewport()
      await page.hover('.cm-lint-marker-error')
      await expect(
        page.getByText('Entity found but it was of the wrong type').first()
      ).toBeVisible()
    })

    // TODO: enable once the app issues are resolved
