...
test.describe('Command bar tests', () => {
  ...
  test('Pattern Circular 3D from command bar', async ({
    page,
    homePage,
    scene,
    cmdBar,
...
solid001 = extrude(sketch001, length = 5)`
    ...
    await test.step('Open Pattern Circular 3D command from toolbar', async () => {
      // Click the Circular Pattern button in the toolbar
    ...
    await test.step('Select solid and configure basic parameters', async () => {
      // Select the solid by clicking on the extrude line
      await editor.selectText('extrude(sketch001, length = 5)')
      await cmdBar.progressCmdBar()

      // Should be on instances step
      await cmdBar.expectState({
        stage: 'arguments',
        commandName: 'Pattern Circular 3D',
      ...
      // Should now be on the axis parameter step
      await cmdBar.expectState({
        stage: 'arguments',
        commandName: 'Pattern Circular 3D',
        currentArgKey: 'axis',
    ...
    await test.step('Review and submit the command', async () => {
      await cmdBar.expectState({
        stage: 'review',
        commandName: 'Pattern Circular 3D',
        headerArguments: {
  ...
  test('Pattern Circular 3D with optional parameters', async ({
    page,
    homePage,
    scene,
    cmdBar,
...
solid001 = extrude(sketch001, length = 5)`
    ...
    await test.step('Open command and configure required parameters', async () => {
      await toolbar.patternCircularButton.click()
      ...
      // Should be on review stage with default center
      await cmdBar.expectState({
        stage: 'review',
        commandName: 'Pattern Circular 3D',
        headerArguments: {
    ...
    await test.step('Add optional parameters', async () => {
      // Add arcDegrees
      ...
      // Click on the "True" option in the dropdown
      await cmdBar.selectOption({ name: 'True' }).click()
      // No progressCmdBar() needed since selecting true/false is accepted immediately

      // Add useOriginal - don't check intermediate state since boolean selections might auto-advance
      await cmdBar.clickOptionalArgument('useOriginal')

      // Click on the "False" option in the dropdown
      await cmdBar.selectOption({ name: 'False' }).click()
      // No progressCmdBar() needed since selecting true/false is accepted immediately
    })

    await test.step('Review and submit with all parameters', async () => {
      await cmdBar.expectState({
        stage: 'review',
        commandName: 'Pattern Circular 3D',
        headerArguments: {
