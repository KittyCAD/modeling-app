...
test.describe('Share link tests', () => {
  test(`Open in desktop app with 2000-long code works non-Windows`, {
    tag: ['@web', '@macos', '@linux'],
  }, async ({ page }) => {
  ...
  test(`Open in desktop app with 1000-long code works on Windows`, {
    tag: ['@web', '@windows'],
  }, async ({ page }) => {
    test.skip(process.platform !== 'win32')
    await closeOnboardingModalIfPresent(page)
  ...
  test(`Open in desktop app with 2000-long code doesn't work on Windows`, {
    tag: ['@web', '@windows'],
  }, async ({ page }) => {
    test.skip(process.platform !== 'win32')
    await closeOnboardingModalIfPresent(page)
  ...
  test('should prefill demo project name on web', { tag: ['@web'] }, async ({
    page,
  }) => {
    await closeOnboardingModalIfPresent(page)
