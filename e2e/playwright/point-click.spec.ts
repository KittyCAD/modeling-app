...
`
  ...
  const setTransformCases: { mode: 'right-click menu' | 'command' }[] = [
    {
      mode: 'right-click menu',
    },
    {
      mode: 'command',
    },
  ]
  setTransformCases.map(({ mode }) => {
    test(`Set transform through ${mode}`, async ({
      context,
      page,
      homePage,
      scene,
