...
test.describe(
  'GUI fuzz exploration: symmetric extrusion',
  { tag: ['@web', '@gui-fuzz'] },
  () => {
    test.use({
      userFeatures: [
    ...
    test('extrude a point-and-click rectangle symmetrically', async ({
      cmdBar,
      editor,
      page,
      scene,
      ...
      try {
        ...
        await test.step('Configure an 8 mm symmetric extrusion', async () => {
          ...
          await captureGuiFuzzStep(page, testInfo, 2, 'one-sided-review')

          await cmdBar.clickOptionalArgument('symmetric')
          await cmdBar.selectOption({ name: 'On', exact: true }).click()
          await expect(page.locator('#review-form')).toBeVisible()
          await expect(
            page.getByRole('button', { name: /symmetric\s*:\s*true/i })
          ).toBeVisible()
          await captureGuiFuzzStep(page, testInfo, 3, 'symmetric-review')
        })

        await test.step('Submit and verify the symmetric body', async () => {
          await cmdBar.submit()
          await scene.settled()
          await editor.expectEditor.toContain('extrude(')
          await editor.expectEditor.toContain('symmetric = true')
          ...
          await captureGuiFuzzStep(page, testInfo, 4, 'symmetric-body')
        })
      } finally {
