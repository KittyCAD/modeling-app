...
test.describe(
  'GUI fuzz exploration: chamfer apply and edit',
  { tag: ['@web', '@gui-fuzz'] },
  () => {
    test.use({
      userFeatures: [
    ...
    test('chamfer a point-and-click extrusion edge and edit its length', async ({
      cmdBar,
      editor,
      page,
      scene,
      ...
      try {
        ...
        await test.step('Create a rectangular extrusion', async () => {
          ...
          await captureGuiFuzzStep(page, testInfo, 2, 'body-before-chamfer')
        })
        ...
        await test.step('Apply a 1 mm chamfer', async () => {
          ...
          const lengthInput = cmdBar.argumentInput.locator('[contenteditable]')
          await expect(lengthInput).toBeVisible()
          await lengthInput.fill('1mm')
          await captureGuiFuzzStep(page, testInfo, 4, 'chamfer-length-entered')

          await cmdBar.progressCmdBar()
          await expect(page.locator('#review-form')).toBeVisible()
          await captureGuiFuzzStep(page, testInfo, 5, 'chamfer-review')
          await cmdBar.submit()
          await scene.settled()

          await editor.expectEditor.toContain('chamfer(')
          await editor.expectEditor.toContain(/length\s*=\s*1/)
          await expect(
            await toolbar.getFeatureTreeOperation('Chamfer', 0)
          ).toBeVisible()
          await captureGuiFuzzStep(page, testInfo, 6, 'chamfer-applied')
        })

        await test.step('Edit the chamfer length to 0.5 mm', async () => {
          const chamferOperation = await toolbar.getFeatureTreeOperation(
            'Chamfer',
            0
          )
          await chamferOperation.dblclick({ button: 'left' })
          await cmdBar.expectCommandName('Chamfer')

          const lengthInput = cmdBar.argumentInput.locator('[contenteditable]')
          await expect(lengthInput).toBeVisible()
          await lengthInput.fill('0.5mm')
          await captureGuiFuzzStep(page, testInfo, 7, 'edited-length-entered')

          await cmdBar.progressCmdBar()
          await expect(page.locator('#review-form')).toBeVisible()
          await captureGuiFuzzStep(page, testInfo, 8, 'edited-chamfer-review')
          await cmdBar.submit()
          await scene.settled()

          await editor.expectEditor.toContain(/length\s*=\s*0\.5/)
          ...
          await setGuiFuzzIsometricView(page)
          await page.waitForTimeout(600)
          await captureGuiFuzzStep(page, testInfo, 9, 'edited-chamfer-body')
        })
      } finally {
