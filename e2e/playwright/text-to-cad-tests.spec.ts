...
async function sendPromptFromCommandBarAndSetNewProject(
  ...
  await test.step(`Send prompt from command bar: ${promptStr}`, async () => {
    ...
    await cmdBar
      .selectOption({ name: 'Create Project using Zookeeper' })
      .click()

    await cmdBar.expectState({
      commandName: 'Create Project using Zookeeper',
      stage: 'arguments',
 ...
 *
 */
test.describe('Mocked Zookeeper API tests', { tag: ['@skipWin'] }, () => {
