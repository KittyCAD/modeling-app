...
test.describe('Mocked Text-to-CAD API tests', { tag: ['@skipWin'] }, () => {
  ...
  async function mockPageTextToCAD(page) {
      await page.route('https://api.dev.zoo.dev/ai/text-to-cad/glb?kcl=true', async route => {
        const json = {
      ...
      await page.route('https://api.dev.zoo.dev/user/text-to-cad/*', async route => {
        const json = {
          'mocked': true,
  ...
  test(
    ...
    async ({ context, page }, testInfo) => {
      ...
      await expect(page.getByText('Successfully deleted "my-project-name"')).toBeVisible()
  ...
  test(
    ...
    async ({ context, page }, testInfo) => {
      ...
      await expect(page.getByText('No Projects found, ready to make your first one?')).toBeVisible()
    })
