...
test.describe('Modeling dialog export', {
...
extrude001 = extrude(region001, length = 12mm)`
    ...
    await expect(storage.locator('option:checked')).toHaveText('binary')
    await storage.selectOption('embedded')
    await dialog
    ...
    await expect(page.getByText('Exported successfully').first()).toBeVisible()
