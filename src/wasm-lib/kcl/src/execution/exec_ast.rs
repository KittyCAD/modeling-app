...
fn update_memory_for_tags_of_geometry(
    ...
    for (tag_declarator, arg_sr) in tag_declarator_args {
        ...
        exec_state.mut_memory().add_tag(&tag.value, tag.clone(), *arg_sr)?;
    }
    ...
    match result {
        KclValue::Sketch { value: ref mut sketch } => {
            for (_, tag) in sketch.tags.iter() {
                exec_state.mut_memory().update_tag_if_defined(&tag.value, tag.clone());
            }
        }
        KclValue::Solid(ref mut solid) => {
            for value in &solid.value {
                if let Some(tag) = value.get_tag() {
                    ...
                    exec_state.mut_memory().update_tag_if_defined(&tag.name, t.clone());
