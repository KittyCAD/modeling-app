...
async fn test_updating_kcl_lsp_files() {
    ...
    assert_eq!(
        server.code_map.inner().await.get("file:///test.kcl").unwrap(),
        "test".as_bytes()
    );
    ...
    assert_eq!(
        server.code_map.inner().await.get("file:///test2.kcl").unwrap(),
        "test2".as_bytes()
    );
    ...
    assert_eq!(
        server.code_map.inner().await.get("file:///test3.kcl").unwrap(),
        "changed".as_bytes()
    );
    ...
    assert_eq!(
        server.code_map.inner().await.get("file:///test4.kcl").unwrap(),
        "".as_bytes()
    );
...
fn myFn = (param1) => {
    ...
    // Get the token map.
    let token_map = server.token_map.inner().await.get("file:///test.kcl").unwrap().clone();
    assert!(token_map != vec![]);

    // Get the ast.
    let ast = server.ast_map.inner().await.get("file:///test.kcl").unwrap().clone();
...
async fn test_kcl_lsp_on_change_update_ast() {
    ...
    assert_eq!(
        ast,
        server.ast_map.inner().await.get("file:///test.kcl").unwrap().clone()
    );
    ...
    assert!(ast != server.ast_map.inner().await.get("file:///test.kcl").unwrap().clone());

    // Make sure we never updated the memory since we aren't running the engine.
    assert!(server.memory_map.inner().await.get("file:///test.kcl").is_none());
}
...
async fn serial_test_kcl_lsp_on_change_update_memory() {
    ...
    // Get the memory.
    let memory = server.memory_map.inner().await.get("file:///test.kcl").unwrap().clone();
    ...
    assert_eq!(
        memory,
        server.memory_map.inner().await.get("file:///test.kcl").unwrap().clone()
    );
    ...
    assert!(memory != server.memory_map.inner().await.get("file:///test.kcl").unwrap().clone());
}
...
const part001 = cube([0,0], 20)
    ...
    // Get the tokens.
    let tokens = server.token_map.inner().await.get("file:///test.kcl").unwrap().clone();
...
async fn serial_test_kcl_lsp_code_with_parse_error_and_ast_unchanged_but_has_diagnostics_reparse() {
  ...
  |> ^^^things(3.14, %)"#;
    ...
    // Get the ast.
    let i = server.ast_map.inner().await;
    let ast = i.get("file:///test.kcl");
    assert!(ast.is_none());
...
const part001 = startSketchOn('XY')
  ...
  |> extrude(3.14, %)"#;
    ...
    assert!(ast != crate::ast::types::Program::default());
    // Get the memory.
    let i = server.memory_map.inner().await;
    let memory = i.get("file:///test.kcl");
...
const NEW_LINT = 1"#
    ...
    // Get the symbols map.
    let i = server.symbols_map.inner().await;
    let symbols_map = i.get("file:///test.kcl");
