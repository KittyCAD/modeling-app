...
pub async fn assert(args: Args) -> Result<MemoryItem, KclError> {
    ...
    args.make_user_val_from_f64(inner_assert(data, &description, args.clone()).await?)
}
...
pub async fn assert_lt(args: Args) -> Result<MemoryItem, KclError> {
    ...
    args.make_user_val_from_f64(inner_assert_lt(left, right, &description, args.clone()).await?)
}
...
pub async fn assert_gt(args: Args) -> Result<MemoryItem, KclError> {
    let (left, right, description): (f64, f64, String) = args.get_data()?;
    args.make_user_val_from_f64(inner_assert_gt(left, right, &description, args.clone()).await?)
}
...
pub async fn assert_lte(args: Args) -> Result<MemoryItem, KclError> {
    let (left, right, description): (f64, f64, String) = args.get_data()?;
    args.make_user_val_from_f64(inner_assert_lte(left, right, &description, args.clone()).await?)
}
...
pub async fn assert_gte(args: Args) -> Result<MemoryItem, KclError> {
    let (left, right, description): (f64, f64, String) = args.get_data()?;
    args.make_user_val_from_f64(inner_assert_gte(left, right, &description, args.clone()).await?)
}
