...
async fn setup(code: &str, name: &str) -> Result<(ExecutorContext, Program, uuid::Uuid)> {
    ...
    let MemoryItem::SketchGroup(sketch_group) = execution.memory.get(name, SourceRange::default()).unwrap() else {
        anyhow::bail!("part001 not found in memory: {:?}", execution.memory);
    };
