...
describe('updateModelingState', () => {
  ...
  it('does not focus code when execution fails before refreshing the artifact graph', async () => {
    const ast = {} as Node<Program>
    const selections: Selections = {
      graphSelections: [],
      otherSelections: [],
    ...
    const kclManager = {
      updateAst: vi.fn(async () => ({ newAst: ast, selections })),
      updateEditorWithAstAndWriteToFile: vi.fn(),
      executeAst: vi.fn(async () => {
    ...
    expect(kclManager.selectRange).not.toHaveBeenCalled()
  })
})
