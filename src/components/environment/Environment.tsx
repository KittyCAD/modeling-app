...
export function EnvironmentDescription () {
  ...
  return (
    ...
    className="font-bold text-xs uppercase px-2 py-1 rounded-sm"
      ...
      <ul>
      <li className='flex flex-col px-2 py-4 gap-1 last:mb-0 '><p className='font-bold'>API</p> <p>{environment?.API_URL}</p></li>
      <li className='flex flex-col px-2 py-4 gap-1 last:mb-0 '><p className='font-bold'>Site</p> <p>{environment?.SITE_URL}</p></li>
      <li className='flex flex-col px-2 py-4 gap-1 last:mb-0 '><p className='font-bold'>Websocket</p> <p>{environment?.WEBSOCKET_URL}</p></li>
      </ul>
      </div>
  )
}
