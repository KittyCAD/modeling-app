...
function compute(artifactGraph, wasmInstance) {
  const idsWithTypes = Array.from(artifactGraph).map((a) => {
    ...
    return { id, type, codeRef, codeRefToIds}
  })
  return idsWithTypes
}
...
function codeRangeToEventInfo(codeRef, wasmInstance) {
  ...
  const asIfItWasSelected = EditorSelection.create([
    EditorSelection.range(selections.graphSelections[0].codeRef.range[0], selections.graphSelections[0].codeRef.range[1])
  ])
...
function parseOutAllSelectAddEntities (eventInfo) {
  ...
  const selectAdds = engineEvents.filter((event)=>{return event.cmd.type === 'select_add'})
  let ids = []
          ...
          className={`cursor-pointer ${hightlightMyId ? highlightMajor : ''}`}
            ...
            >
              ...
              {a.codeRefToIds.map((id) => {
                ...
                return (<div className={`ml-4 cursor-pointer ${highlightMyId ? highlightMinor : ''}`}
                  onClick={() => {
                    selectionFromId(id)
                    setSelectedId(id)
                  }}
