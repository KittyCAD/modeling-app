...
const canPasswordProtectShareLinks = (subOrErr: undefined | SubscriptionsOrError): boolean => {
  if (subOrErr === undefined || typeof subOrErr === 'number' || err(subOrErr)) return false
...
export const ShareButton = () => {
  ...
  const allowPassword = canPasswordProtectShareLinks(billingContext.subscriptionsOrError)
         ...
         <div className="flex flex-col px-2">
          <div className="flex flex-row gap-1 items-center">
            ...
            <input
              ...
              type="text" placeholder="Set a password"
            />
          </div>
          <div>
            <label htmlFor="org-only" className="pl-1 inline-flex items-center">
              <input disabled={!allowOrgRestrict} checked={isRestrictedToOrg} onChange={_ => setIsRestrictedToOrg(!isRestrictedToOrg)} type="checkbox" name="org-only" className="form-checkbox"/>
              <span className={`text-xs ml-2 ${allowOrgRestrict ? 'cursor-pointer' : 'cursor-not-allowed text-chalkboard-50'}`}>Org. only access</span>
            </label>
            { !allowOrgRestrict && <Tooltip>
              Upgrade to Organization to use this feature.
            </Tooltip> }
          </div>
          <button disabled={disabled} onClick={onShareClickProOrOrganization}>Generate</button>
         </div>
      </Popover.Panel> }
    </Popover>
  )
