...
export const AppHeader = ({ children }: React.PropsWithChildren) => {
    return (
        <header className="py-1 px-5 bg-chalkboard-10 border-b border-chalkboard-30 flex justify-between">
            <button onClick={() => alert('A project menu pane is coming soon!')}>
                <img src="/kitt-arcade-winking.svg" alt="KittyCAD App" className="h-9 w-auto" />
            ...
            {children || (
                ...
                <button onClick={() => alert("An account menu pane is coming soon!")}>Account</button>
            )}
        </header>
    )
}