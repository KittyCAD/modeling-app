...
function toFileStat(stat: LegacyFileStat): FileStat {
  return {
...
/**
 * Adapt the current `fsZds` implementation to the Effect capability.
 *
 * The adapter is intentionally the only module aware of the legacy combined
 * path/filesystem object, making it possible to replace each platform backing
 * without changing Effect programs above it.
 */
export function makeLegacyFileSystem(
  backing: IZooDesignStudioFS
): FileSystemService {
  const stat = (path: string) =>
    tryLegacy('stat', path, () => backing.stat(path)).pipe(
  ...
  const readFile = (path: string) =>
    tryLegacy('read-file', path, () => backing.readFile(path))

  const makeDirectory = (path: string) =>
    tryLegacy('create-directory', path, () =>
  ...
  const writeFile = (path: string, contents: Uint8Array) => {
    ...
    return makeDirectory(parent).pipe(
      Effect.zipRight(
        tryLegacy('write-file', path, () => backing.writeFile(path, bytes))
      ),
      Effect.asVoid
    )
  }
  ...
  const service: FileSystemService = {
    ...
    makeDirectory,
    readDirectory: (path) =>
      tryLegacy('read-directory', path, () => backing.readdir(path)).pipe(
...
export const legacyFileSystemLayer = (backing: IZooDesignStudioFS) =>
  Layer.succeed(FileSystem, makeLegacyFileSystem(backing))
