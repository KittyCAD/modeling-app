      ...
      if (file.children) {
        for (let entry of file.children) {
          if (entry.name.endsWith('.kcl')) {
            return {
              defaultFilePath: window.electron.path.join(
                projectDir,
                entry.name
              ),
              didCreate: false,
        ...
        return { defaultFilePath, didCreate: true }
      }
    }
  }

  if (!file.children) {
    return { defaultFilePath: file.name, didCreate: false }
  }

  return { defaultFilePath, didCreate: false }
}

function buildProjectsBasedOffFileEntries(
...
export async function getProjectInfo(projectPath: string): Promise<Project> {
  ...
  const projectName = getStringAfterLastSeparator(projectPath)
  let project = buildProjectsBasedOffFileEntries(
    results,
  ...
  project.default_file = defaultFilePath
  if (didCreate) {
    let results = await window.electron.globDir(starMatchedPath)
    project = buildProjectsBasedOffFileEntries(
      results,
      projectDirectoryPath
    )[0] || {
      path: projectPath,
