...
const prepareToEditParameter: PrepareToEditCallback = async ({
  ...
  const nodeToEdit = pathToNodeFromRustNodePath(operation.nodePath).slice(
    0,
    -1,
  );
  ...
  if (
    ...
    if (segId) {
      ...
      if (!err(segResult)) {
        ...
        const codeRefResult = getEdgeCutConsumedCodeRef(
          artifact,
          artifactGraph,
        );
        if (
...
const prepareToEditHelix: PrepareToEditCallback = async ({
  ...
  if (edgeRefPayload) {
    const { retrieveEdgeSelectionsFromSingleEdgeRef } =
...
async function prepareToEditScale({
  ...
  if (operation.labeledArgs.factor) {
    const res = await extractKclArgument(
      code,
      operation,
      "factor",
      rustContext,
