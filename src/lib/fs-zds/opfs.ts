...
const walk = async (targetPath: string, onTargetNode?: (part: string) => void): Promise<undefined | FileSystemDirectoryHandle | FileSystemFileHandle> => {
...
type ReadFileReturn<T> = T extends ('utf8' | { encoding: 'utf-8' }) ? string : Uint8Array

const readFile = async <T extends ReadFileOptions>(path: string, options?: T): Promise<ReadFileReturn<T>> => {
  const handle = await walk(path)
  if (handle === undefined) return Promise.reject('ENOENT')
...
const writeFile = async (path: string, data: Uint8Array<ArrayBuffer>, options?: any) => {
  const parts = path.split('/')
  const parent = parts.slice(0, -1).join('/')
  const handle = await walk(parent)
  if (handle === undefined) return Promise.reject('ENOENT')
  if (handle instanceof FileSystemFileHandle) return Promise.reject('EISFILE')
  const handleFile = (await handle).getFileHandle(parts.slice(-1)[0], { create: true })
