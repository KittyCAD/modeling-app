...
export function parseMlCopilotModesResult(
  ...
  if (typeof modesResponse !== 'object' || modesResponse === null) {
    return null
  }
...
export const mlEphantManagerMachine = setup({
  ...
  actions: {
    assignApiToken: assign(({ event }) => {
      assertEvent(event, MlEphantManagerTransitions.SetApiToken)
      return {
  ...
  actors: {
    [MlEphantManagerStates.Setup]: fromPromise(
      async (
        ...
        const maybeReplayedExchanges: Exchange[] = []
        ...
        return await new Promise<Partial<MlEphantManagerContext>>(
          (onFulfilled, onRejected) => {
            ...
            ws.addEventListener(
              'message',
              (event: MessageEvent<string | ArrayBufferLike>) => {
                ...
                if ('replay' in response) {
                  for (const byteMessage of response.replay.messages) {
                    ...
                    if (!isMlCopilotServerMessage(responseReplay)) {
                      continue
                    }

                    // Don't show deltas because they are aggregated in the end_of_stream
                    if ('delta' in responseReplay) {
                      continue
                    }
                    ...
                    const lastExchange = maybeReplayedExchanges.slice(
                      -1
                    )[0] ?? {
                      responses: [],
                    }
    ...
    [MlEphantManagerTransitions.MessageSend]: fromPromise(
      async (
    ...
    [MlEphantManagerStates.ContinueCheck]: fromPromise(
      async (
    ...
    [MlEphantManagerTransitions.Cancel]: fromPromise(
      async (
    ...
    [MlEphantManagerTransitions.Interrupt]: fromPromise(
      async (
