...
export default async function getCurrentProjectFile(
  ...
  if (stats.mode & fsZdsConstants.S_IFDIR) {
    ...
    if (kclFiles.length === 0) {
      ...
      try {
        ...
        const newFileContents = changeKclVersion(
          '',
          DEFAULT_KCL_VERSION,
          wasmInstance
        )
        if (err(newFileContents)) return newFileContents
  ...
  if (shouldWrapExtension(extension)) {
    ...
    try {
      ...
      const settings = changeKclVersion('', DEFAULT_KCL_VERSION, wasmInstance)
      if (err(settings)) return settings
      const content = `${settings}// This file was automatically generated by the application when you
