...
/** Locks are UI-session state, but survive closing and reopening the pane. */
export const lockedNamedViewKeysSignal = signal<ReadonlySet<string>>(new Set())

/**
 * Artifact IDs are regenerated by execution and can shift when source is
 * inserted earlier in a file. A project/module/name tuple is stable across
 * those executions and still keeps same-named imported views distinct.
 */
export function namedViewSessionKey(
  projectPath: string,
  view: KclNamedView
): string {
  const moduleKey = moduleKeyOf(view.modulePath) ?? `Module:${view.moduleId}`
  return JSON.stringify([projectPath, moduleKey, view.artifact.name])
}
...
export function resetNamedViewSession(): void {
  ...
  lockedNamedViewKeysSignal.value = new Set()
}
