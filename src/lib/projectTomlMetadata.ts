...
function cloudEnvironmentFromSectionName(sectionName: string) {
  if (!sectionName.startsWith('cloud.')) {
    return undefined
  }

  const environmentName = sectionName.slice('cloud.'.length)
  return parseTomlString(environmentName) ?? environmentName
}
...
export function getCloudProjectIdFromProjectTomlContents(
  ...
  const section = getSectionRange(lines, (sectionName) => {
    const sectionEnvironment = cloudEnvironmentFromSectionName(sectionName)
    if (!sectionEnvironment) {
      return false
    }
    return environmentName ? sectionEnvironment === environmentName : true
  })
  if (!section) {
    return undefined
  }

  const projectId = getKeyValueFromSection(lines, section, 'project_id')
