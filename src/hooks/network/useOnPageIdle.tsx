...
import { engineCommandManager, kclManager, sceneInfra, useSettings } from '@src/lib/singletons'
import { EngineCommandManagerEvents } from '@src/network/utils'
import { useEffect, useRef, useState } from 'react'
import { useModelingContext } from '../useModelingContext'
import { trap } from '@src/lib/trap'

export const useOnPageIdle = ({ startCallback }: { startCallback: () => void }) => {
    const settings = useSettings()
    const [streamIdleMode, setStreamIdleMode] = useState(
        settings.app.streamIdleMode.current
    )
    const { state: modelingMachineState } =
        useModelingContext()
