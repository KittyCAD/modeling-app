...
- Check async ownership where state changes, not just where work starts. Background jobs, engine callbacks, Zookeeper runs, queues, and handoffs must not create stale state.
- For sketch, constraint, region, solver/frontend, and point-and-click changes, simple happy-path sketches are not enough. Look for validation on arcs, tangent constraints, partially constrained sketches, hidden support geometry and dragged clusters.
