 ...
 */
export const kclAnalysisExecutor: Executor = {
  id: 'kcl.analysis',
  order: 100,
  ...
  async run(request) {
    ...
    try {
      ...
      return {
        ...
        diagnostics: issuesToDiagnostics(issues ?? [], request.contents.length),
      }
    } catch (thrown) {
      ...
      return {
        requestId: request.requestId,
        diagnostics: thrownErrorToDiagnostics(thrown, request.contents.length),
      }
    }
  },
}
...
export default defineRegistryItem({
  id: 'kclAnalysis',
  provides: [provide(executorsValueSpec, kclAnalysisExecutor)],
})
