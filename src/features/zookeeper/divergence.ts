...
/**
 * How far our copy of a file has drifted from the agent's copy of it.
 *
 * This is the missing argument to `rebaseEdits`. The agent computes its edits
 * against its own document; we hold a different one, because the user has been
 * typing and because our earlier applications were themselves rebased. A
 * `ChangeSet` from the agent's document to ours is exactly what turns the
 * agent's coordinates into ours, and its `length` is the agent's document length —
 * which is the invariant `rebaseEdits` checks.
 *
 * **Why this and not "everything since the baseline, minus the agent's own
 ...
 * - An **agent** edit `A`, in the agent's coordinates, moves the *start* of the
 *   divergence. `divergence.map(A)` re-expresses the drift against the agent's
 *   new document. Taking `A` rather than the rebased edit we actually dispatched
 *   is the part that is easy to get backwards: `A` is what moved the agent's
 *   document, and the agent's document is what the divergence is measured from.
 */
export interface DivergenceLedger {
  /**
   * Start tracking a path, with our document and the agent's believed identical.
   *
   * Called when a turn's baseline is captured for a file. `length` is that
   * baseline's length, in the coordinates the agent will answer in.
   */
   ...
   * document the divergence currently ends at. See `recordAgent` for why that is
   * reported rather than ignored.
   */
  recordLocal(path: string, changes: ChangeSet): boolean
  /**
   * An agent edit that has been applied, in the **agent's** coordinates.
   *
   * That is the pre-rebase edit, as `deriveChanges` produced it — not the mapped
   * one that was dispatched.
   *
   * Returns false, changing nothing, when `changes` does not start from the
   * agent's document as the divergence understands it. The check exists because
   ...
   */
  recordAgent(path: string, changes: ChangeSet): boolean
  /**
...
export function createDivergenceLedger(): DivergenceLedger {
  ...
  return {
    ...
    recordLocal(path, changes) {
      ...
      /*
       * An untracked path is one no turn has a baseline for, so there is no
    ...
    recordAgent(path, changes) {
      const current = drift.get(path)
      if (current === undefined) return true
      if (changes.empty) return true
      // An agent edit applies to the agent's document, where the drift starts.
      if (changes.length !== current.length) return false
      /*
       * `before: true` is not decoration, and getting it wrong is invisible until
       * the agent and the user insert at the *same* offset — which is common,
       ...
       * agent's edit with the default flag, which orders it *after* a local
       * insertion at the same point — the user's text was already there, so the
       * agent's arriving text follows it. That fixes this side: the local change
       * must be ordered *before* the agent's, which is what `true` says. Flip
