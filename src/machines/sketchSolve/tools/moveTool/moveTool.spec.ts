...
describe('createOnDragCallback', () => {
  ...
  it('should preserve the last good geometry for a degenerate constrained preview', async () => {
    const getIsSolveInProgress = vi.fn(() => false)
    const setIsSolveInProgress = vi.fn()
    const getLastSuccessfulDragFromPoint = vi.fn(() => new Vector2(0, 0))
    const setLastSuccessfulDragFromPoint = vi.fn()
    const getDraggedEntityId = createDraggedEntityIdGetter(7)
    const lineStart = createPointApiObject({ id: 1, x: 0, y: 0, owner: 3 })
    const lineEnd = createPointApiObject({ id: 2, x: 4, y: 0, owner: 3 })
    const line = createLineApiObject({ id: 3, start: 1, end: 2 })
    const arcCenter = createPointApiObject({ id: 4, x: 4, y: 2, owner: 7 })
    const arcStart = createPointApiObject({ id: 5, x: 4, y: 0, owner: 7 })
    const arcEnd = createPointApiObject({ id: 6, x: 6, y: 2, owner: 7 })
    ...
    const collapsedLineEnd = createPointApiObject({
      id: 2,
      x: 0.01,
      y: 0,
      owner: 3,
    })
    const degenerateDelta = createSceneGraphDelta([
      lineStart,
      collapsedLineEnd,
      line,
      arcCenter,
    ...
    const editSegments = vi.fn(() =>
      Promise.resolve({
        kclSource: { text: 'degenerate code' },
        sceneGraphDelta: degenerateDelta,
      })
    )
    const onNewSketchOutcome = vi.fn()
    const dragSnappingDeps = createDragSnappingDeps()
    ...
    await callback({
      intersectionPoint: {
        twoD: new Vector2(0, 1),
        threeD: new Vector3(0, 1, 0),
      },
      selected: undefined,
      mouseEvent: createTestMouseEvent(),
      intersects: [],
