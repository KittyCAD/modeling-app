...
describe('systemIOMachine - XState', () => {
  ...
  describe('desktop', () => {
    ...
    describe('when duplicating projects', () => {
      it('flushes a pending text edit after the project has closed', async () => {
        await moduleFsViaModuleImport({
          type: StorageName.NodeFS,
          options: {},
        })
        const projectDirectory = await mkdtemp(
          path.join(tmpdir(), 'zds-duplicate-closed-project-')
        )
        const sourceProject = path.join(projectDirectory, 'source')
        const textFilePath = path.join(sourceProject, 'notes.txt')
        ...
        try {
          ...
          await writeFile(textFilePath, 'original')
          await writeFile(
            path.join(sourceProject, 'project.toml'),
            'title = "!!!"\ndefault_file = "main.kcl"\n'
          )
          await openActiveTextFile(textFilePath)
          const debounceTimer = vi
            .spyOn(globalThis, 'setTimeout')
            .mockImplementationOnce(() => 0 as never)
          scheduleActiveTextFileWrite(textFilePath, 'pending edit')
          debounceTimer.mockRestore()
          appInstanceInThisFile.project = undefined
          ...
          expect(result).toMatchObject({ name: '1', title: '!!!-1' })
          await expect(
            readFile(
              path.join(projectDirectory, result.name, 'notes.txt'),
              'utf-8'
            )
